...
class MultiHeadedLocalAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x = torch.stack([self.attn(query[:, i], key[:, i], value[:, i], 
                    input_mask=mask) for i in range(self.h)], dim=1) 
...
class VTP_wrapper(nn.Module):
    ...
    def forward(self, faces, mask):
        ...
        lens = mask.long().sum((1, 2)) 
        indiv_faces = []
        for l, fs in zip(lens, faces):
            indiv_faces.append(fs[:l])

        face_tokens = torch.cat(indiv_faces, dim=0) 

        face_tokens = self.hwposition(face_tokens)

        face_embeddings = self.encoder(face_tokens) 

        pooling_weights = F.softmax(self.pooler(face_embeddings), dim=1)
        self.face_attn_weights = pooling_weights

        face_embeddings = (face_embeddings * pooling_weights).sum(1) 
